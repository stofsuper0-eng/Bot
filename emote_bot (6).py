...
class EmoteBot(BaseBot):
    ...
    async def on_start(self, session_metadata: SessionMetadata):
        ...
        # ── Move to saved position immediately on reconnect ──────────
        if is_reconnect and self.bot_pos and not self.follow_target_id:
            async def _restore_position():
                ...
                if self.is_connected and self.bot_pos and not self.follow_target_id:
                    try:
                        ...
                        print(f"[Reconnect] ✅ Moved to saved pos ({bp['x']}, {bp['y']}, {bp['z']})")
                    except Exception as e:
                        print(f"[Reconnect] ⚠️ Could not restore position: {e}")
