...
class EmoteBot(BaseBot):
    ...
    async def _dawya_emote_loop(self):
        ...
        print("[Dawya] Loop started, looking for @_Dawya_...")
        try:
            while self.is_connected:
                ...
                if not self._dawya_target_id:
                    ...
                    try:
                        ...
                        usernames = [u.username for u in room_users]
                        print(f"[Dawya] Searching in room ({len(room_users)} users): {usernames}")
                        
                        for user in room_users:
                            if user.username == "_Dawya_":
                                self._dawya_target_id = user.id
                                print(f"[Dawya] ✅ FOUND @_Dawya_ with ID {user.id}")
                                break
                        else:
                            if room_users:
                                print(f"[Dawya] ⚠️ @_Dawya_ not in room yet")
                    except Exception as e:
